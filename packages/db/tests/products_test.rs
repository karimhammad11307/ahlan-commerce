...
fn get_test_db_url() -> String {
    std::env::var("DATABASE_URL").unwrap_or_else(|_| "postgres://ahlan:ahlan_dev@localhost:5432/ahlan_commerce".to_string())
}
...
async fn test_create_and_list_products() {
    ...
    // Test create
    let created = db::products::create_product(&**client, domain_product.clone()).await.expect("Failed to create product");
    assert_eq!(created.title, "Test Product");
    assert_eq!(created.published, true);
    assert!(created.published_at.is_some());

    // Test list
    let products = db::products::list_products(&**client).await.expect("Failed to list products");
    assert!(products.len() > 0);
    
    // Test list published
    let published_products = db::products::list_published_products(&**client).await.expect("Failed to list published products");
